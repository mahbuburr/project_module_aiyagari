function [ U, c, k, K, sim, store ] = get_utility( par, func, method )
%GET UTILITY Calculates the lifetime utility of agents in Aiyagari model
   ...
   grid.dist = linspace(grid.k(1),grid.k(end),grid.dist_no); % grid for distribution of agents - use finer grid
    ...
    while d1>1e-6 && iter<50 % loop for aggregate problem
        ...
        if strcmp(method.sim ,'simulation')
            ...
            sim.u(func.C(sim.k)<0) = -Inf; % Make sure consumption is always positive.

            % Calculate the life time utility when simulation has converged (about half
            % the size of the simulation).
            U = par.beta.^(1:ceil(ind_no/2))*sim.u((ceil(ind_no/2)+1):end,:);
        ...
        elseif strcmp(method.sim,'histogram')
            ...
            % Calculate aggregate utility from consumption
            U = func.U(sum(sum(store.distribution.*(func.C(k.k)))));

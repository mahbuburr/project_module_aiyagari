...
\framesubtitle{Summary}
  \begin{itemize}
  \item {
  There exists a welfare increase in the mean and in the median.
  }
  \item {
  However, there is disagreement on whether to increase or decrease the unemployment benefits.
  }
  \item {
  Careful: the consumption equivalent is biased towards poorer agents as relative increases for them are higher than for already high-consumption agents.
  }
  \end{itemize}
\end{frame}
...
\framesubtitle{Summary}
  \begin{itemize}
  \item {
  There does not exist a higher welfare state in the mean, only in the median.
  }
  \item {
  If a higher welfare state existed for the mean we could redistribute wealth such that everyone would be better off. \\
  $\rightarrow$ Pareto improvement.
  }
  \item {
  One can clearly see the friction between unemployed and employed.
  }
  \end{itemize}
\end{frame}

\begin{frame}{Cash vs. Consumption Equivalent}
\centering{\includegraphics[scale=0.6]{cash_cons_equiv}}
\end{frame}
...
\subsection{The cost of the business cycle}
...
\begin{frame}{Solution}
  ...
  \item {
  \citeauthor{krussel_smith97} \cite{krussel_smith97} suggest an algorithm to solve the issue by an approximation, which could be summerized as ”rational adaptive expectations”.  
  }
...
\begin{frame}{Welfare loss of the business cycle}
  ...
  \item {
  Surpress Krussel - Smith algorithm such that the economy is initialy at the middle between good and bad state. i. e. $z = 1, U = 0.07$ and the probability of being employed next period is equal to the half in the 2 aggregate states. 
  }
...
\begin{frame}{Wellfare loss of the business cycle}{Calculate lifetime utilities}
  \begin{itemize}
  ...
  \item {
  Take the ergodic distribution and put it in aggregate economy framework. Solve the individual problem and caluclate the \textbf{EXPECTED} lifetime utility. 
  }

  \[\textrm{Compare: }U^1(k_i, e_i) \textrm{ and } \mathbb{E}U^2(e_i, k_i, K^{ss}, z)\]
...
\begin{frame}{Results}
  ...
  \item {
  The median consumption equvalent is: 0.1603\%.
  }
...
\begin{frame}{Another application}{Transition between two steady states for 10k individuals}
...
\begin{frame}{Numerical problems and solutions}
  ...
  \item {
  How to solve the individual problem? Trade off between accuracy and speed.  
  }
...
\begin{frame}{Conclusion}
  ...
  \item {
  Government policies could influece the society significantly!
  }

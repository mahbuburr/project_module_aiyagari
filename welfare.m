...
life_U = par.beta.^(1:ceil((ind_no-1)/2))*sim.u(ceil((ind_no-1)/2):end,:);
life_U_rep = sum(par.beta.^(1:floor((ind_no-1)/2))*func.U(func.C(K.guess)));
...
func.c_equiv_bench = @(U_2) ((U_2.*(1-par.sigma).*(1-par.beta)+1)...
    ./(life_U_rep*(1-par.sigma)*(1-par.beta)+1)).^(1/(1-par.sigma));
test = func.c_equiv_bench(life_U); 
histogram(test)
